...
const Search = () => {
    ...
    const isSnippetsOpen = useSelector(
        (state) => state.responseReducer.isSnippetsOpen,
    );
    ...
    const onSearchValue = async (e) => {
        ...
        if (inputValue.length > 0) {
            try {
                ...
                if (res.totalItems) {
                    dispatch(
                        snippetsItemsAction(getData.getBookInfo(res.items)),
                    );
                }
